...
size_t	format(t_props props, char* buf, int idx, int sign)
{
	...
	if (!(props.flags & FLAGS_LEFT)) {
		if (props.width && (props.flags & FLAGS_ZEROPAD) && (!sign || (props.flags
		...
		while ((props.flags & FLAGS_ZEROPAD) && (len < props.width) && (len < PRINTF_NTOA_BUFFER_SIZE)) {
			buf[len++] = '0';
		}
	}

	if (props.flags & FLAGS_HASH) {
		if (!(props.flags & FLAGS_PRECISION) && len && ((len == props.precision) || (len == props.width))) {
			len--;
			if (len && (props.base == 16U)) {
				len--;
			}
			if ((props.base == 16U) && !(props.flags & FLAGS_UPPERCASE) && (len < PRINTF_NTOA_BUFFER_SIZE)) {
				buf[len++] = 'x';
			}
			else if ((props.base == 16U) && (props.flags & FLAGS_UPPERCASE) && (len < PRINTF_NTOA_BUFFER_SIZE)) {
				buf[len++] = 'X';
			}
...
size_t reverse_print(t_props props, char* buf, int len, int idx)
{
	...
	if (!(props.flags & FLAGS_LEFT) && !(props.flags & FLAGS_ZEROPAD)) {
		for (size_t i = len; i < props.width; i++) {
			ft_putchar(' ');
			idx++;
		}
	}
